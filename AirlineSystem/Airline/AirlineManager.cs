...
namespace AirlineTicketSystem
{
    public class AirlineManager
    {
        ...
        public int GetTotalFlights() => flights?.Count ?? 0;
        public int GetTotalPassengers() => passengers?.Count ?? 0;
        public int GetTotalTickets() => tickets?.Count ?? 0;
        public decimal GetTotalRevenue() => tickets?.Sum(t => Convert.ToDecimal(t.TicketPrice)) ?? 0m;
                ...
                while ((line = sr.ReadLine()) != null)
                {
                    ...
                    try
                    {
                        if (cols.Length >= 6)
                        {
                            ...
                            if (addFlight(new Flight(flightNumber, departure, destination, departureTime, availableSeats, status)))
                                importCount++;
                            else
                                skipCount++;
                        }
        ...
        public void ImportPassengerFormCSV(string filename)
        {
            try
            {
                if (!File.Exists(filename))
                {
                    Console.WriteLine($"Passenger file {filename} not found, skipping...");
                    return;
                }
                ...
                while ((line = sr.ReadLine()) != null)
                {
                    ...
                    try
                    {
                        if (cols.Length >= 5)
                        {
                            ...
                            char gender = Convert.ToChar(cols[4].Trim());
                            if (addPassenger(new Passenger(name, email, gender, age, phone)))
                                importCount++;
                            else
                                skipCount++;
                        }
            ...
            {
                Console.WriteLine($"Import passenger failed: {ex.Message}");
            }
        }
